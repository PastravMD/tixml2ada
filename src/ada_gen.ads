...
package Ada_Gen is
   ...
   function Is_Interfaces_Hierarchy (Spec : Ada_Spec) return Boolean;

   procedure Write_Spec (Spec : Ada_Spec; Output_Dir : String);
   ...
   function Is_Similar (T1, T2 : Ada_Type) return Boolean is abstract;
   ...
   procedure Add (Spec : in out Ada_Spec; Elt : Ada_With_Clause);
   procedure Add (Spec : in out Ada_Spec; Elt : Ada_Comment'Class);
   procedure Add (Spec : in out Ada_Spec; Elt : Ada_Pragma);

   procedure Add_No_Check (Spec : in out Ada_Spec; Elt : Ada_Type'Class);
   --  Adds a new type definition.
   --  If a previous type has the same Id and different definition, then
   --  a constraint_error is raised

   procedure Add (Spec : in out Ada_Spec; Elt : in out Ada_Type'Class);
   --  Adds a new type definition.
   --  If a previous type has the same Id and different definition, then
   --  Elt's id is modified

   procedure Add (Spec : in out Ada_Spec; Elt : Ada_Constant_Value);
   procedure Add (Spec : in out Ada_Spec; Elt : Ada_Instance);

   function New_With_Clause
     (Pkg         : String;
      Use_Visible : Boolean := False) return Ada_With_Clause;

   function New_Comment (Comment : String) return Ada_Comment;

   function New_Comment_Box (Comment : String) return Ada_Comment_Box;
   ...
   procedure Add_Size_Aspect (Elt : in out Ada_Type'Class; Size : Natural);
   ...
   procedure Add_Aspect (Elt : in out Ada_Type'Class; Aspect : String);
   --  Generic method to add aspects to type definition

   overriding function Is_Similar (T1, T2 : Ada_Type_Scalar) return Boolean;
   overriding function Is_Similar (T1, T2 : Ada_Subtype_Scalar) return Boolean;
   overriding function Is_Similar (T1, T2 : Ada_Type_Array) return Boolean;
   overriding function Is_Similar (T1, T2 : Ada_Type_Enum) return Boolean;
   overriding function Is_Similar (T1, T2 : Ada_Type_Record) return Boolean;
   overriding function Is_Similar (T1, T2 : Ada_Type_Union) return Boolean;
   ...
   procedure Add_Address_Aspect (Elt : in out Ada_Instance; Val : String);

   procedure Add_Aspect (Elt : in out Ada_Instance; Aspect : String);
